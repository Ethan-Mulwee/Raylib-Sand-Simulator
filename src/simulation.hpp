...
void IntializeGrid(int rows, int columns) {
    std::vector<std::vector<std::shared_ptr<Cell>>> cells(std::vector<std::vector<std::shared_ptr<Cell>>>(rows, std::vector<std::shared_ptr<Cell>>(columns, std::make_shared<Cell>(BLACK, true))));
};
